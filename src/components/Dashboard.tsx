...
const YOUR_API_KEY = 'key_1d2025c27c6328b3f9840255e4df'; // Replace with your actual API key
...
const saveCallAnalytics = async (callId: string) => {
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      // Check if analyticsData contains the expected data
      if (analyticsData && Object.keys(analyticsData).length > 0) {
        break; // Data is available, exit the loop
      } else {
        console.log('Analytics data not ready yet, retrying...');
        analyticsData = null; // Reset analyticsData
      }
    }
    ...
    if (user) {
      ...
      // Add the new call analytics to the existing analytics
      analytics[callId] = analyticsData;

      // Update the document with the new analytics data
...
export default function Dashboard() {
  ...
  // Keep the ref updated with the latest currentCallId
  useEffect(() => {
